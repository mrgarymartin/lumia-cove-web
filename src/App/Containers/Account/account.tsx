  ...
  componentDidMount() {
	...
	getSubscriptionInfo = async () => {
		try {
			...
			const response = await getRequest(`subscription/info`, { headers: { Authorization: `Bearer ${this.props.auth.token}` }});
	...
	continueSubscription = async() => {
		...
		try {
			const response = await getRequest(`subscription/resume`, { headers: { Authorization: `Bearer ${this.props.auth.token}` }});
			console.log('response: ', response.data);
			this.getSubscriptionInfo();
		} catch (err) {
			console.log('err: ', err);
	...
	cancelSubscription = async () => {
		if (window.confirm('Are you sure you would like to cancel your subscription?')) {
			...
			try {
				const response = await getRequest(`subscription/cancel`, { headers: { Authorization: `Bearer ${this.props.auth.token}` }});
				console.log('response: ', response.data);
				this.getSubscriptionInfo();
			} catch (err) {
				console.log('err: ', err);
